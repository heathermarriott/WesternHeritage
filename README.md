...
languages.html is my idea to provided a limited number of translation languages 
(not exactly the same as having the avatar speak in other languages, but easier to implement)

ToDo: 
1) Under settings, when user selects Proto size, display all webpages in dimentions that work for proto (I do not know if this step is necessary.  The curret mockup looks great on my phone in protrait mode which is similar to proto) 
